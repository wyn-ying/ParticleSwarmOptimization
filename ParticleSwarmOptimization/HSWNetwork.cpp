...
network HSWConstruct(int k, double p)//construct a ring network with degree=2k
{
	...
	while (countnum < numofedge*p)
	{
		...
		//swap the reverse direction
		for (auto i = ring[tmp_1].neighborsID.begin();i != ring[tmp_1].neighborsID.end();i++) {
			if (*i == targetID_1) {
				ring[tmp_1].neighborsID.erase(i);
				ring[tmp_1].neighborsID.push_back(targetID_2);
				break;
			}
		}
		for (auto i = ring[tmp_2].neighborsID.begin();i != ring[tmp_2].neighborsID.end();i++) {
			if (*i == targetID_2) {
				ring[tmp_2].neighborsID.erase(i);
				ring[tmp_2].neighborsID.push_back(targetID_1);
				break;
			}
		}
		countnum += 2;
